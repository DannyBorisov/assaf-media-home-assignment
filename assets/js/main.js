...
function canUserSendMessage($messageContent, $username, $contact) {
  if ($.globals.isPendingMsg) {
    consoleLog(
      "you're trying to call sendMessage while another proccess is running",
      { level: 2, type: "error" }
    );
    return false;
  }
  ...
  if (!$.settings.allowSendingMsgs) {
    consoleLog(
      "you're trying to call sendMessage while $.settings.allowSendingMsgs is false",
      { level: 5, type: "error" }
    );
    return false;
  }
...
var sendMessage = async function (
  ...
  if (!canUserSendMessage($msg, $username, $contactId)) {
    return;
  }
...
$(window).on("load", function () {
  ...
  $("body").on("change", "#image-upload", function (e) {
    ...
    if (!files || files.length === 0) {
      consoleLog("No files selected for upload", { level: 0 });
      return;
    }
